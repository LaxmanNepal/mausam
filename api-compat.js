...
const currentFromHourly=(data)=>{
  ...
  const key=hourKey(new Date().toISOString());
  ...
  try{
    ...
    if(u.hostname==='api.open-meteo.com'&&u.pathname==='/v1/forecast'){
      ...
      if(!r.ok){
        ...
        minimal.searchParams.delete('daily');
      ...
      if(!r.ok)return jsonResponse({error:true,reason:'Weather provider returned HTTP '+r.status},200);
      const data=await r.json();
      return jsonResponse(currentFromHourly(data));
    }
