...
let rows=[],sort='risk',province='all';
...
async function scan(){
 ...
 try{
  ...
  rows=rows.map(x=>{const rs=riverStations.filter(r=>match(r,x.name));x.riverDanger=rs.some(r=>r.status==='danger');x.riverWarning=!x.riverDanger&&rs.some(r=>r.status==='warning');x.risk=risk(x);return x});
